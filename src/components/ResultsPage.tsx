...
export function ResultsPage({ results, inputData, backendData, onNavigate }: ResultsPageProps) {
  ...
  const maxLength = Math.max(
    results.pso.convergenceData.length,
    results.ga.convergenceData.length,
    results.ilp.convergenceData.length
  );

  const convergenceComparison = Array.from({ length: maxLength }, (_, i) => ({
    iteration: i,
    PSO: results.pso.convergenceData[i]?.fitness || results.pso.convergenceData[results.pso.convergenceData.length - 1]?.fitness,
    GA: results.ga.convergenceData[i]?.fitness || results.ga.convergenceData[results.ga.convergenceData.length - 1]?.fitness,
    ILP: results.ilp.convergenceData[i]?.fitness || results.ilp.convergenceData[results.ilp.convergenceData.length - 1]?.fitness,
  }));
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <div className="mt-3 inline-flex items-center gap-2 px-3 py-1 bg-blue-50 dark:bg-blue-950 text-blue-700 dark:text-blue-300 rounded-md">
            <AlertCircle className="w-4 h-4" />
            <span className="text-sm">Menampilkan data demo untuk preview</span>
          </div>
        </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div>{results.pso.routes.length}</div>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={400}>
              <LineChart data={convergenceComparison}>
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="PSO" stroke="#ef4444" strokeWidth={2} />
                <Line type="monotone" dataKey="GA" stroke="#3b82f6" strokeWidth={2} />
                <Line type="monotone" dataKey="ILP" stroke="#22c55e" strokeWidth={2} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Rute kendaraan dari solusi terbaik (ILP)</CardDescription>
          </CardHeader>
          <CardContent>
            <Tabs defaultValue="ilp">
              <TabsList className="grid w-full grid-cols-3">
                <TabsTrigger value="pso">PSO</TabsTrigger>
                <TabsTrigger value="ga">GA</TabsTrigger>
                <TabsTrigger value="ilp">ILP</TabsTrigger>
